...
namespace WindowsGSM.Plugins
{
    public class interstellar_rift : SteamCMDAgent // SteamCMDAgent is used because the_forest relies on SteamCMD for installation and update process
    {
        ...
        {
            name = "WindowsGSM.interstellar_rift", // WindowsGSM.XXXX
            author = "Tempus Thales",
            description = "🧩 WindowsGSM plugin for supporting Interstellar Rift Dedicated Server",
            version = "1.0",
            url = "https://github.com/tempusthales/WindowsGSM.interstellar_rift", // Github repository link (Best practice)
            color = "#0a7a04" // Color Hex
        };


        // - Standard Constructor and properties
        public insterstellar_rift(ServerConfig serverData) : base(serverData) => base.serverData = _serverData = serverData;
        private readonly ServerConfig _serverData; // Store server start metadata, such as start ip, port, start param, etc


        // - Settings properties for SteamCMD installer
        public override bool loginAnonymous => false; // Interstellar Rift requires to login steam account to install the server, so loginAnonymous = false
        public override string AppId => "363360"; // Game server appId, Interstellar Rift is 363360


        // - Game server Fixed variables
        public override string StartPath => "IR.exe"; // Game server start path, for the_forest, it is TheForestDedicatedServer.exe
        public string FullName = "Interstellar Rift Dedicated Server; // Game server FullName
